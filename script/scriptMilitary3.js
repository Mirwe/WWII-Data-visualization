...
function createBarChart(subDati, paese, daDove){
    ...
    var xTickValues = ["1940","1941","1942","1943","1944","1945"];
...
function createLineChart(dati){
    ...
    var paesi = ["Germany","United Kingdom","Italy","Soviet Union"];
    ...
    paesi.forEach(function(p){
        ...
        dati.forEach(function(d){
            if(d.Nationality == p && !isNaN(d.DeathsFinal) )
                count+=d.DeathsFinal;
        })
        console.log(count);
        totMortiPaese.push(count);
    })

    h = 400;
    w = 600;

    paesi.forEach(function(paese){
        ...
        var yScale = d3.scaleLinear()
                    .domain([0, d3.max(totMortiPaese, function (d) {
                        return d;
                    })])
                    .range([0, h]);
        ...
        var xScale = d3.scaleLinear()
                ...
                var yAxis = d3.axisLeft(AsseyScale);  
                    ...
                    d3.select('#yAxis2')
                        .attr("transform", "translate("+startBar+",0)")   
                        .call(yAxis);*/
